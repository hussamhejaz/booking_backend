...
function validateOfferData(data, mode = "create") {
  ...
  // if we're creating, these must exist.
  // if we're updating, only validate them if they are being changed.
  ...
  // If either date is present, check date order
  if (data.start_date !== undefined || data.end_date !== undefined) {
    ...
    // only check if we actually have BOTH
...
async function createOffer(req, res) {
  try {
    ...
    // Validate data using create mode
    ...
    if ((calculatedFinalPrice === undefined || calculatedFinalPrice === null) && original_price) {
      if (discount_percentage) {
        calculatedFinalPrice = original_price * (1 - discount_percentage / 100);
...
async function updateOffer(req, res) {
  try {
    ...
    // Validate using update mode (no hard-required fields)
    ...
    // Auto-recalc final_price ONLY IF user touched pricing but didn't send final_price
...
async function getOfferCategories(req, res) {
  try {
    ...
    // Get categories from salon_categories table
