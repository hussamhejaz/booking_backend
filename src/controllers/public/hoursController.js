...
function slotsFromWorkingHours(dayConfig, busyWindows, duration) {
  if (!dayConfig || dayConfig.is_closed || !dayConfig.open_time || !dayConfig.close_time) {
    return [];
  }
...
async function getPublicAvailableHours(req, res) {
  try {
    ...
    const bookingType = type === "home" || home_service_id ? "home" : "salon";
    ...
    if (!resolvedDuration) {
      if (bookingType === "home" && home_service_id) {
        const { data, error } = await supabaseAdmin
          .from("home_services")
          .select("id, duration_minutes, salon_id, is_active")
        ...
        resolvedDuration = data.duration_minutes || 30;
      } else if (service_id) {
        const { data, error } = await supabaseAdmin
          .from("services")
          .select("id, duration_minutes, salon_id, is_active")
    ...
    if (hoursError) {
      console.error("getPublicAvailableHours working hours error:", hoursError);
      return res.status(500).json({
        ok: false,
    ...
    if (bookingError) {
      console.error("getPublicAvailableHours bookings error:", bookingError);
      return res.status(500).json({
        ok: false,
    ...
    if (bookingType === "home" && home_service_id) {
      ...
      if (error) {
        console.error("getPublicAvailableHours home service slots error:", error);
      } else {
        specificSlots = data || [];
      ...
      if (error) {
        console.error("getPublicAvailableHours service slots error:", error);
      } else {
        specificSlots = data || [];
    ...
    if (manualSlotsError) {
      console.error("getPublicAvailableHours manual slots error:", manualSlotsError);
    }
    ...
    if (!availableSlots.length && manualSlots?.length) {
      ...
      if (manualAvailable.length) {
        ...
        slotStrategy = "salon_manual";
      }
    }

    if (!availableSlots.length) {
      availableSlots = slotsFromWorkingHours(dayConfig, busyWindows, resolvedDuration);
    ...
    console.error("getPublicAvailableHours fatal:", err);
    return res.status(500).json({
      ok: false,
      error: "SERVER_ERROR",
      details: err.message,
